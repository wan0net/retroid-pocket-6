...
"$ROOT_DIR/scripts/configure-controls.sh"

if package_installed org.es_de.frontend; then
  "$ROOT_DIR/scripts/configure-es-de.sh"
else
  note 'ES-DE is not installed yet; run make configure-es-de after installing it.'
fi
